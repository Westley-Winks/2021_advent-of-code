...
``` r
df <- read_table("inputs/input_day-2.txt", col_names = c("direction", "magnitude"))
```
...
depth <- df %>% 
  ...
  mutate(magnitude = case_when(
    direction == "up" ~ -magnitude, # if direction is up, make it negative
    direction == "down" ~ magnitude
  )) %>% 
  pull(magnitude) %>% 
  sum()

product <- prod(hor_pos, depth)

print(paste0("Final Product of Horizontal and Depth: ", product))
```

    ## [1] "Final Product of Horizontal and Depth: 1484118"
...
aimed_df <- df %>% 
  ...
  mutate(aim = accumulate(aim_change, sum)) %>% # adds previous elements
  
  mutate(hor_pos_change = case_when(
    direction == "forward" ~ magnitude, 
    TRUE ~ 0
  )) %>% 
  
  mutate(hor_pos = accumulate(hor_pos_change, sum)) %>% 
  
  mutate(depth_change = case_when(
    direction == "forward" ~ magnitude * aim, 
    TRUE ~ 0
  )) %>% 
  
  mutate(depth = accumulate(depth_change, sum))

final_hor_pos <- last(aimed_df$hor_pos)
final_depth <- last(aimed_df$depth)

product <- prod(final_hor_pos, final_depth)

print(paste("Final Product of Horizontal and Depth: ", product))
```

    ## [1] "Final Product of Horizontal and Depth:  1463827010"
...
df <- read_table("inputs/input_day-3.txt", col_names = c("binary"))
# df <- data.frame(binary = c("00100", "11110", "10110", ""))
```

``` r
split_df <- df %>% 
  separate(binary, into = c("empty", LETTERS[1:12]), sep = "", remove = TRUE) %>% 
  select(-empty)

gamma_rate <- list()
epsilon_rate <- list()

for (i in colnames(split_df)) {
  
  gamma_rate_new <- split_df %>% 
    group_by(split_df[[i]]) %>% # group by either 1 or 0
    count() %>% 
    arrange(n) %>% 
    extract2(2, 1) # most abundant bit in column
  
  epsilon_rate_new <- split_df %>% 
    group_by(split_df[[i]]) %>% 
    count() %>% 
    arrange(n) %>% 
    extract2(1, 1) # least abundant bit in column

  gamma_rate <- append(gamma_rate, gamma_rate_new)
  epsilon_rate <- append(epsilon_rate, epsilon_rate_new)
}

gamma_rate <- str_c(gamma_rate, collapse = "") %>% 
  strtoi(base = 2)

epsilon_rate <- str_c(epsilon_rate, collapse = "") %>% 
  strtoi(base = 2)

power_consumption <- gamma_rate * epsilon_rate

print(paste0("Power Consumption: ", power_consumption))
```

    ## [1] "Power Consumption: 2640986"
